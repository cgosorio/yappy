...
class SimpleExp(Yappy):
    ...
    def __init__(self, no_table=0, table='saexp.tab'):
        ...
        tokenize = [(r"\s+", ""),
                    (r"\d+", lambda x: ("id", int(x))),
                    (r"\+", lambda x: (x, x)),
                    (r"\*", lambda x: (x, x)),
                    (r"\(|\)", lambda x: (x, x))]
        Yappy.__init__(self, tokenize, grammar, table, no_table, tmpdir='/tmp')
...
class SimpleExp1(Yappy):
    ...
    def __init__(self, no_table=0, table='saexp1.tab', tabletype=LALRtable,
        ...
        tokenize = [(r"\d+", lambda x: ("id", int(x))),
...
class SimpleExp2(SimpleExp1):
    """A parser for simple arithmetic expressions with prec and associativity"""
...
class SimpleExpAmb(SimpleExp2):
    """A parser for simple arithmetic expressions with an ambiguous grammar"""
    def __init__(self, no_table=0, table='expamb.tab', tabletype=LALRtable,
                 noconflicts=1, expect=0):
        grammar = grules([
